...
import { cancelQueueJob, VALID_QUEUE_NAMES, type ValidQueueName } from '@/lib/queue-management'
...
export async function POST(
  ...
  try {
    ...
    return await withRoles(request, [WebUserRole.ADMIN], async (admin) => {
      ...
      const result = await cancelQueueJob(queue, jobId)
      ...
      await logAuditEvent({
        ...
        entityId: jobId,
        details: { action: 'cancel', queue: queueName },
      })
